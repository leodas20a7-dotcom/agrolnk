...
export async function uploadProduceImage(file, folder = 'listings') {
  ...
  try {
    // 1. First generate compressed Base64 representation
    const base64Data = await compressImageToBase64(file, 1200, 0.82);
    ...
    try {
      ...
      console.info('Storage bucket not active, using embedded image storage:', storageErr);
    }

    // 3. Fallback: Base64 data URL is 100% saved into DB row and renders everywhere!
    return base64Data;
  } catch (err) {
